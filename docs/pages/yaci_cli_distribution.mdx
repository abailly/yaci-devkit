...
You'll find three configuration files within the `config` folder: `application.properties`, `node.properties`, `download.properties`.

<Callout type="warning">
In order for `yaci-cli` to pick its configuration files, it needs to be run with its working directory set to the parent directory of `config` folder.
</Callout>
